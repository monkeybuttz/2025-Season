...
def Train():
    ...
    # Filter out any empty DataFrames from the list
    NFL_team_data = [df for df in NFL_team_data if not df.empty]
    ...
    MODEL_FILENAME = 'Models/' + NFL_WEEK + '/gbm.sav'
...
def printOutcomes(model):
    ...
    for index, row in data.iterrows():
        ...
        if team_a_win_prob > team_b_win_prob:
            data.at[index, 'GBM'] = team_a
        else:
            data.at[index, 'GBM'] = team_b
...
if __name__ == '__main__':
    ...
    model = pickle.load(open('Models/' + NFL_WEEK + '/gbm.sav', 'rb'))
